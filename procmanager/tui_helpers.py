...
def format_seconds(seconds):
    ...
    if -10 < seconds < 60:
        ...
        rem_minutes_in_seconds = int(seconds % 3600)
        minutes = f'{int(rem_minutes_in_seconds / 60)}m '
    elif seconds > 3600 * 24:
        days = f'{int(seconds / (3600 * 24) )}d '  # (86400)
        rem_hours_in_seconds = int(seconds % (3600 * 24))
        hours = f'{int(rem_hours_in_seconds / 3600)}h '
        rem_minutes_in_seconds = int(rem_hours_in_seconds % 60)
        minutes = f'{int(rem_minutes_in_seconds / 60)}m '
        rem_seconds = int(rem_minutes_in_seconds % 60)
    return f'{days}{hours}{minutes}{rem_seconds}'
        

def generate_log_table(top_down=False, ptk=False, limit=None, offset=None, last=None):
    properties = ['id', 'status', 'started_at', 'finished_at', 'pid', 'running_length'] #jobname
    rows = []

    now = datetime.now()
    for ji in procmanager.db.list_job_instances(top_down=top_down, limit=limit, offset=offset, last=last):
...
if __name__ == '__main__':
    def test_format_seconds():
        results = [format_seconds(53), format_seconds(72), format_seconds(8262), format_seconds(85302), format_seconds(60 * 60 * 24 * 10 + 13)]
        print(results)
    test_format_seconds()
    quit()
